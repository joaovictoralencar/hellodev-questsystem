...
namespace HelloDev.QuestSystem.QuestGraph.Editor.Nodes
{
    ...
    /// - Asset Mode (checked): Reference an existing TaskLocation_SO asset
    /// - Define Mode (unchecked): Define task inline with ports + dynamic condition ports
    /// </remarks>
    [Serializable]
    public class TaskLocationBlock : TaskBlockBase
    {
        #region Option Names

        private const string OPT_USE_TASK_ASSET = "UseTaskAsset";
        private const string OPT_LOCATION_TASK_ASSET = "LocationTaskAsset";
        ...
        public override Task_SO TaskAsset => UseTaskAsset
            ? GetOptionValue<TaskLocation_SO>(OPT_LOCATION_TASK_ASSET)
            : null;
        ...
        /// <inheritdoc/>
        protected override string GetAssetOptionName() => OPT_LOCATION_TASK_ASSET;
        ...
        protected override void DefineAssetModeOption(IOptionDefinitionContext context)
        {
            context.AddOption<bool>(OPT_USE_TASK_ASSET)
                ...
                .WithTooltip("Check to use an existing TaskLocation_SO asset.\nUncheck to define task inline.");

            if (UseTaskAsset)
            {
                context.AddOption<TaskLocation_SO>(OPT_LOCATION_TASK_ASSET)
                    .WithDisplayName("Task Asset")
                    .WithTooltip("Reference to a TaskLocation_SO asset");
            }
        }
        ...
        protected override void AddTypeSpecificPorts(IPortDefinitionContext context)
        {
            if (UseTaskAsset)
            {
                context.AddInputPort<TaskLocation_SO>(PORT_TASK_ASSET)
                    .WithDisplayName("Task Asset")
                    .Build();
            }
            else
        ...
        {
            var task = UnityEngine.ScriptableObject.CreateInstance<TaskLocation_SO>();
            return task;
        }
