...
{
    /// <summary>
    /// Configuration ScriptableObject for managing save slot naming conventions.
    /// Tracks the current active slot and provides slot-based naming.
    /// When playing on slot X, autosaves go to "autosave-X" instead of overwriting a single autosave file.
    /// </summary>
    [CreateAssetMenu(fileName = "SaveSlotConfig", menuName = "HelloDev/Quest System/Save Slot Config")]
    public class SaveSlotConfig_SO : ScriptableObject
    {
        ...
        public void SetActiveSlot(int slotIndex)
        {
            ...
            {
                Debug.LogWarning($"[SaveSlotConfig] Invalid slot index: {slotIndex}. Must be -1 to {maxSlots - 1}.");
                return;
            }
        ...
        public string GetManualSlotKey(int slotIndex)
        {
            ...
            {
                Debug.LogWarning($"[SaveSlotConfig] Invalid slot index: {slotIndex}. Must be 0 to {maxSlots - 1}.");
                return null;
            }
