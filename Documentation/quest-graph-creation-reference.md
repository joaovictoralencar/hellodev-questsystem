...
Task nodes represent individual tasks within a TaskGroup. There are type-specific task nodes for each task type:

| Node Class | Task Type | Extra Fields |
|------------|-----------|--------------|
| `TaskBoolNode` | TaskBool_SO | (none) |
| `TaskIntNode` | TaskInt_SO | RequiredCount |
| `TaskStringNode` | TaskString_SO | TargetValue |
| `TaskLocationNode` | TaskLocation_SO | (none) |
| `TaskDiscoveryNode` | TaskDiscovery_SO | RequiredDiscoveries |
| `TaskTimedNode` | TaskTimed_SO | TimeLimit, FailQuestOnExpire |

> **Note:** The generic `TaskNode` class is deprecated. Use type-specific nodes for new graphs.

#### Task Mode: Auto-Detection

Task nodes automatically detect their mode based on the **Task Asset** field:

- **Asset mode**: When Task Asset is assigned → uses the referenced Task_SO
- **Define mode**: When Task Asset is empty → uses inline data to create a new task

> **Note:** All fields are always visible in the inspector. The "Inline:" prefixed fields are only used when Task Asset is empty. This provides a simple UX without needing conditional field visibility.

#### Asset Mode (Reference Existing Task)

When Task Asset is filled, the node uses the referenced asset:

```yaml
- rid: <TaskBoolNode_wrapper>
  type: {class: UserNodeModelImp, ...}
  data:
    m_InputConstantsById:
      m_KeyList:
      - In
      - __option_BoolTaskAsset
      m_ValueList:
...
- rid: <Task_SO_constant>
  type: {class: 'Constant`1[[HelloDev.QuestSystem.ScriptableObjects.TaskBool_SO, HelloDev.QuestSystem]]', ...}
  data:
    m_Value: {fileID: 11400000, guid: <task_guid>, type: 2}
```

#### Define Mode (Inline Task Creation)

When Task Asset is empty, the inline fields define the task:

```yaml
- rid: <TaskIntNode_wrapper>
  type: {class: UserNodeModelImp, ...}
  data:
    m_InputConstantsById:
      m_KeyList:
      - In
      - __option_IntTaskAsset         # Empty (null) triggers Define mode
      - __option_DevName
      - __option_DisplayName
      - __option_TaskDescription
      - __option_Conditions
      - __option_FailureConditions
      - __option_RequiredCount        # TaskInt-specific
      m_ValueList:
      - rid: <TaskFlow_constant>
      - rid: <Task_SO_constant>       # null reference
      - rid: <string_constant>        # DevName: "Kill 10 Goblins"
      - rid: <LocalizedString_constant>
      - rid: <LocalizedString_constant>
      - rid: <ConditionList_constant>
      - rid: <ConditionList_constant>
      - rid: <int_constant>           # RequiredCount: 10
```

**Common Options (all task types):**
- `[Type]TaskAsset`: Reference to Task_SO (if set, Asset mode; if empty, Define mode)
- `DevName`: Internal developer name (used when Task Asset is empty)
- `DisplayName`: LocalizedString for UI (used when Task Asset is empty)
- `TaskDescription`: LocalizedString description (used when Task Asset is empty)
- `Conditions`: List<Condition_SO> for completion (used when Task Asset is empty)
- `FailureConditions`: List<Condition_SO> for failure (used when Task Asset is empty)

**Type-Specific Options (used when Task Asset is empty):**
- `TaskIntNode`: `RequiredCount` (int) - number of times to complete
- `TaskStringNode`: `TargetValue` (string) - value to match
- `TaskDiscoveryNode`: `RequiredDiscoveries` (int) - 0 = all conditions
- `TaskTimedNode`: `TimeLimit` (float), `FailQuestOnExpire` (bool)

**Ports:**
- Input: `In` (TaskFlow)
- Output: `Then` (TaskFlow) - for sequential task chains

### Backward Compatibility: Legacy TaskNode

Existing graph files may use the generic `TaskNode` class, which has been deprecated but remains fully functional for backward compatibility.

**Key differences:**
- `TaskNode` uses `__option_TaskAsset` with `Constant<Task_SO>` (base type)
- Type-specific nodes use typed options like `__option_BoolTaskAsset` with `Constant<TaskBool_SO>`

**Migration path:**
- Existing graphs using `TaskNode` continue to work without modification
- For new graphs, use type-specific nodes for better type safety
- To migrate an existing graph, recreate task nodes in the editor (YAML surgery is not recommended)

**Why backward compatibility works:**
- `TaskNode` inherits from `TaskBaseNode`
- The `GraphToQuestConverter` uses `TaskBaseNode` type throughout
- Both legacy and new nodes are processed identically
...
TaskGroupNode.Tasks → TaskBoolNode.In (or other task node type)
...
- Use type-specific task nodes (TaskBoolNode, TaskIntNode, etc.) for new graphs
- Use **Asset mode** for reusable tasks across quests
- Use **Define mode** for quick prototyping or one-off tasks
...
- Use the deprecated generic `TaskNode` class (use type-specific nodes)
