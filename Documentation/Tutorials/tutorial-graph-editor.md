...
1. Right-click canvas → Add a type-specific task node (e.g., **TaskBoolNode**, **TaskIntNode**)
2. In the node, set **Task Asset** to an existing Task_SO (e.g., `SO_Task_TalkToMerchant`)
3. Connect the TaskNode to your stage's workflow

**Option A2: Create Inline Tasks (No Pre-Existing Asset)**
1. Right-click canvas → Add a type-specific task node
2. Leave **Task Asset** empty to use "Define mode"
3. Fill in the **Inline:** fields (Dev Name, Display Name, Conditions, etc.)
4. On export, a Task_SO sub-asset is automatically created inside the .quest file
